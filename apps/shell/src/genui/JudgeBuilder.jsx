/* JudgeBuilder.jsx — generative-UI input component (tool-judge_builder, PHASE2-C).
   ...
   invariant (mirror CriterionBuilder): surfacing the card writes NOTHING; the human's "Create judge"
  ...
  return (
    <Card className="my-3">
      <CardHeader>
        ...
        <span className="font-[family-name:var(--font-mono)] text-[10.5px] text-muted-foreground">a new voice on the council (audited)</span>
      </CardHeader>
      <CardContent className="flex flex-col gap-3.5">
        <Field label="Role id">
          <Input value={role} onChange={(e) => setRole(e.target.value)} placeholder="escalation_judge" aria-label="judge role" aria-invalid={role.length > 0 && !roleValid} />
        </Field>
        {role.length > 0 && !roleValid && <span className="text-[10.5px] text-[color:var(--accent-ink)]">Role id must be lower_snake (e.g. escalation_judge)</span>}

        <Field label="Lens (codes this judge may raise)">
          <div className="flex flex-col gap-1 max-h-44 overflow-y-auto pr-1" data-testid="judge-lens-list">
            {available.length === 0 ? (
              <span className="text-[11px] text-muted-foreground">No codes in the active pack yet — author a criterion first.</span>
            ) : (
              available.map((code) => {
                ...
                return (
                  <div key={code} className="flex items-center gap-2 rounded-[var(--radius-sm)] border border-border bg-background px-2.5 py-1.5">
                    ...
                    <label className="flex items-center gap-1.5 cursor-pointer" style={ownedNoLens ? { color: "var(--amber)" } : undefined}>
                      ...
                      <span className="text-[10.5px]" style={{ color: ownedNoLens ? "var(--amber)" : "var(--muted)" }}>own (one-strike)</span>
                    </label>
                  </div>
                );
              })
        ...
        {!ownerEmitOk && (
          <span data-testid="owner-emit-guard" className="text-[10.5px]" style={{ color: "var(--amber)" }}>
            ⚠ owner↔emit: {ownedNotInLens.join(", ")} {ownedNotInLens.length > 1 ? "are" : "is"} owned but not in the lens — a judge can only own a code it can raise. Add it to the lens, or un-own it.
          </span>
        )}

        <Field label="Model (pick-from-pool)">
          <select
            value={modelId}
          ...
          >
            <option value="">— default deployment (no explicit binding) —</option>
            {pool.map((m) => (
        ...
        <Field label="Role-prompt seed (optional)">
          <textarea
            value={rolePrompt}
            ...
            rows={3}
            placeholder="You are the escalation reviewer. Raise a code only when…"
        ...
        <Field label="Rationale (audit why)">
          <Input value={rationale} onChange={(e) => setRationale(e.target.value)} placeholder="why this judge" aria-label="audit rationale" />
        </Field>
      ...
      <CardFooter>
        ...
        >
          {persist.state !== "idle" ? persist.msg : returned ? "judge created ✓" : "POST /v1/judges · owner↔emit + snapshot gated"}
        </span>
        <Button className="ml-auto" size="sm" onClick={apply} disabled={!valid || persist.state === "saving"}>
          {persist.state === "saving" ? "Creating…" : "Create judge"}
        </Button>
      </CardFooter>
    </Card>
  );
