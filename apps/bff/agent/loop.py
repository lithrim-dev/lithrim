...
_SYSTEM_PROMPT = (
    ...
    "the prose — e.g. transcript.text or patient_profile.active_medications> so the card opens "
    "PRE-FILLED with a deterministic presence_check draft (you may also pass suggested_params to "
    "override). ALWAYS say the pre-filled params are a DRAFT they edit + Save — you only PROPOSE; the "
    "human's Save is the only write and the suggestion never decides a verdict. (When YOU already "
    "know the contract_type + params, use add_grounding_contract — the agent-composes twin — instead.)\n"
