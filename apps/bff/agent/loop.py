...
_SYSTEM_PROMPT = (
    ...
    "author_contract with source_hint=<the chart path to check the flagged term against, lifted from "
    "the prose — e.g. transcript.text or patient_profile.active_medications>. The SYSTEM then fills "
    "the CORRECT deterministic presence_check params (med_source / dosage_regex / token_min_len / "
    "noise_tokens) from source_hint — so do NOT hand-compose the params dict yourself (you'll get the "
    "keys wrong; presence_check needs med_source + dosage_regex, NOT needle/haystack). Pass "
    "suggested_params ONLY if you already know the exact executor keys. ALWAYS say the pre-filled "
    "params are a DRAFT they edit + Save — you only PROPOSE; the human's Save is the only write and "
    "the suggestion never decides a verdict. (When YOU already know the contract_type + params, use "
