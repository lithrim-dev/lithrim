...
3. **The case gets one of three states, with its evidence attached.** Cleared, flagged, or escalated to a person (engine verdicts `PASS`, `BLOCK`, `WARN`). A check that cannot decide is surfaced as **inconclusive**, never silently resolved. A judge's confidence never clears a signal; only a check does. The decision is computed once in the engine and rides every record, so the report pane and the inline verdict card title a case Flagged, Cleared, or Needs a person with the reason under it, the same call `make queue` prints.
