...
| **S-BS-10** | **ontology flag-source fork — 4 flags outside `taxonomy_snapshot.json`** | **medium** | **DECIDED 2026-05-31 = Option A (snapshot-authoritative + lint gate); WS-2 deliverable-0** |

## Load-bearing context the next monitor MUST know

1. **S-BS-10 — DECIDED 2026-05-31 (user) = Option A: snapshot-authoritative + lint gate. This is WS-2 deliverable-0 (harness-only, do it first).** Diagnosis CONFIRMED: the 4 flags (`FABRICATED_CONSENT_SCOPE`, `MALAFFI_CODE_PROPAGATION`, `MISSING_DUAL_CODING`, `WRONG_PATIENT_INFO`) are *defined-but-untiered* in the backend itself (in `safety_flags.py`, absent from `compliance_council.py` TIER_1/2/3 + `_TIER1_OWNERS`) — already inert in grading. **Implementation:** ontology gradeable set = the 19 in-snapshot tiered codes (tier+owner); `seed_ontology.py` cross-checks every seeded flag against `taxonomy/taxonomy_snapshot.json` and partitions **gradeable (in-snapshot)** vs **reference (out-of-snapshot, `tier=null, gradeable=false`)**; grounding/`_rescore` operate on gradeable-only; the seed lint FAILS if a *gradeable* flag is outside the snapshot. `safety_flags.py` remains a prose source for in-snapshot codes only. **Rejected:** Option B (backfill — needs a backend tier/owner clinical decision per flag; separate cross-repo cycle) and Option C (extend `snapshot_taxonomy.py` to carry prose and drop the `safety_flags.py` coupling — noted as eventual cleanup, bigger schema change, not now). Two doc-drifts to be aware of: the snapshot has **no literal `KNOWN_TAXONOMY_CODES` key** (CLAUDE.md names one) — the de-facto known set is the tier-union (19) + structural (5); a future doc/snapshot tidy could add the explicit key.
