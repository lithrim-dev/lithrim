...
> **⚠ POST-CRUD-1 CITATION REFRESH (monitor, 2026-06-07, against HEAD `a15be6e`):** CRUD-1 (`893d7a7..eff4911`) shifted `app.py`/`tools.py` line numbers and **landed the delete primitive** FLAG-1 reuses. Corrected anchors (the §1 entries below predate CRUD-1 — re-grep to confirm, but start here): `_validate_ontology` **`app.py:797`** (was 709) · `put_ontology_endpoint` **`app.py:821`** (was 732) · `_build_tool_context` **`app.py:1017`** · `_author_flag` **`app.py:1090`** (was 989) · `PAID_KEYS` **`tools.py:75`** (was 69) · `author_flag_handler` **`tools.py:199`** (was 189) · `AUTHOR_FLAG_SCHEMA` `tools.py:47` (stable). **`delete_with_audit` NOW EXISTS at `lithrim_bench/harness/audit.py:219`** (+ `delete_judge` `judges.py:120`, `delete_agent` `config.py:199`) — D3's flag-delete reuses `delete_with_audit` directly. **Tool count is 9** (CRUD-1 added `delete_judge`) → `create_flag` is the **10th** (resolves the D2 "9 or 10" hedge). `ontology.py` (FlagDefinition:42, gradeable_flags:107, is_reference:116), `packager.py` (package_case:125, production_owners_of-check:150), `taxonomy.py:50` — all **stable** (CRUD-1 untouched). Also heed **S-BS-98** (the judge store is GLOBAL/per-role): a reference flag created on one agent's ontology working copy is per-agent (ontology is per-agent), but judges are global — keep the "clean" claim honest.
