...
| **WS-2** | Generalize `:8002` | inject `council_config`/`ontology` into `PipelineRequest` (Optional, additive) → domain-agnostic grading | **ready** (next) · **must reconcile Q4.1** (taxonomy flag-source) first |
...
| S-BS-10 | ontology flag-source fork — 4 flags (`FABRICATED_CONSENT_SCOPE`, `MALAFFI_CODE_PROPAGATION`, `MISSING_DUAL_CODING`, `WRONG_PATIENT_INFO`) seeded from council `safety_flags.py` are **absent from `taxonomy/taxonomy_snapshot.json`** (the CLAUDE.md contract-of-record) | medium | **open — opened WS-1 critique Q4.1; CONFIRMED.** Inert in WS-1 (WS-0 case uses only in-snapshot flags). **WS-2 PRE-CONDITION:** decide flag-source authority (backfill the 4 into the snapshot, or make the snapshot the tier/membership authority + council source = definitions-only) BEFORE the ontology drives live grading. Fix loc: `scripts/seed_ontology.py` source choice + `taxonomy/taxonomy_snapshot.json`. |
...
1. **PRIORITY-0 (gates WS-2): reconcile the ontology flag-source — new seam S-BS-10 / WS-1 critique Q4.1.** 4 flags (`FABRICATED_CONSENT_SCOPE`, `MALAFFI_CODE_PROPAGATION`, `MISSING_DUAL_CODING`, `WRONG_PATIENT_INFO`) are seeded from the council `safety_flags.py` but **absent from `taxonomy/taxonomy_snapshot.json`** — which `CLAUDE.md` makes the contract-of-record. Decide flag-source authority BEFORE the ontology drives live grading in WS-2: either backfill the 4 into the snapshot, or have `seed_ontology.py` treat the snapshot as the tier/owner/membership authority and the council source as definitions-only. This is a `taxonomy/` contract decision — user-confirm before it lands.
