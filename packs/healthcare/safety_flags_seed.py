"""Healthcare safety-flag seed — the 23 clinical flag definitions that seed
``packs/healthcare/ontology.json`` (relocated here in CE-PACK-6b-CLEAN, D2-a).

This is healthcare GENERATION content: ``scripts/seed_ontology.py`` reads
``SAFETY_FLAG_DEFINITIONS`` + ``TAXONOMY_VERSION`` to (re)generate the pack ontology
byte-identically — the same posture as the PACK-5 generators seeding the
by-construction corpus. It moved OUT of the domain-agnostic core
(``lithrim_bench/runtime/council/safety_flags.py``, deleted in this cycle) so the core
council carries zero clinical content (the generic-CE demarcation).

Pydantic-only import (no ``openai``) — safe on the offline core, loaded BY FILE PATH
(the pack-code-by-path convention; cf. ``harness/pack.py`` load_pack_floors / generators).

NB the regenerated ontology's ``_provenance.flag_source`` still names the historical
core path (``lithrim_bench/runtime/council/safety_flags.py:SAFETY_FLAG_DEFINITIONS``):
that literal is the authoring-origin RECORD, kept verbatim so the moat-frozen ontology
stays byte-identical (D2-a) — it is NOT a live import path. The live source is THIS file.
"""
