...
- **BFF API surface** — **RATIFIED 2026-06-01 (WS-5-BFF; fresh-critic).** v1 of the "judge-capability API" is locked as: `POST /v1/run-eval` (the eval-report `composite` + a degenerate-N=1 `calibration_check` **folded into the response** — no discrete get-report, since `run_eval.run` has no run-id index), `GET /v1/corpus`, `GET /v1/ontology` (**read-only**), `GET /health`. **`PUT`/write-ontology DEFERRED to WS-5c/d** (the phase that wires an ontology editor — locking an unexercised write that would clobber the committed `clinical_v1.json` was rejected at plan-review). This as-locked shape is the contract of record; it supersedes the earlier "run-eval / get-report / get-corpus / read-write-ontology" sketch.
