...
1. **GROUND-FLOOR-1** — offline `record_presence` (string-match `snomed_core`); ships now, $0, proven. + the 1 paid live-confirmation run. **✅ LANDED 2026-06-09 (offline core; the paid run SPLIT → GROUND-FLOOR-1b).** `RecordPresence` wraps `InRowTool` in `harness/grounding.py`, registered in `_CONTRACT_EXECUTORS` (wires both `ground()` post-consensus and the `signals.py` withstands-gate pre-consensus); the `FABRICATED_HISTORY` / `record_presence` contract is declared in `clinical_v1.json` (floor coverage 1/19 → 2/19). Demo-pair flip (clean `aaecd73c3bcf` BLOCK→PASS, viol `1bd0f10dc7b5` stays BLOCK) + **16/16** non-regression (12 inject stand / 4 clean suppressible) + non-SOAP/empty-PMH guards are test-pinned offline (`tests/test_ground_floor1.py`, $0). The paid live-confirmation run is **deferred to GROUND-FLOOR-1b** (re-scoped: design an honest live test — the captured over-fire is a *different* case firing `HALLUCINATED_DETAIL`/`INCOMPLETE_DOCUMENTATION`, not `FABRICATED_HISTORY`, and S-BS-74 shows the clean twin is unanimously approved, so a live FH flip likely won't reproduce; honest-Δ). Disposition: **S-BS-104** (the over-fire — now suppressible *iff* a FABRICATED_HISTORY ever fires on a record-grounded clean note); **S-BS-112** (ATTACH-VALIDATORS now has a second real record-grounded contract). New seam **S-BS-113** (pre-existing `owner_roles` drift between the seed sources and the committed ontology — a clean re-seed would sweep 3 unrelated owner additions; contract hand-added to keep the diff additive-only; needs a separate re-snapshot/owner-reconcile).
...
5. **CONTRACT-AUTHOR** *(generated contracts — the validator+transformer unification; brought in AT TIME, see gates)* — the same `jute_dspy` generate→score→refine→freeze loop that already authors VALIDATORS (`jute_gen` ∈ `_FLOOR_CONTRACT_TYPES`, applied by `_run_floor`, pinned by hash) authors the **EXTRACTION** side too: a new `jute_transform` contract-type registering a generated, frozen JUTE tree-program that extracts data points across one or more **structured** artifacts + relational joins — replacing hand-coded Python extractors (e.g. `record_presence`'s `extractor: "soap_pmh_items"` = `InRowTool.extract_pmh_items`). **Trust-model split (load-bearing):** a transformer is output-inspectable; a validator must be by-construction-gated (it silently mis-grades, it is in the verdict path). A generated extractor that **feeds** a verdict-flipping Contract inherits the validator's burden → it gets a hard structural output-invariant (zero-null / `count==expected`) ON TOP of the downstream gate (the silent wrong-join risk — a mis-join returns `null`, not an error). **Boundary:** JUTE = structured-artifact extraction + cross-artifact joins (strong); free-text stays Python/LLM-extract-then-validate (no regex-match). **Smallest cut / de-risk:** formalize the proven pure-JUTE narrative normalizer as the first `jute_transform` — `jute_dspy` *generates* it (does the loop converge on a transformer, not just a validator?), freeze + register + structural-gate; one $0/deterministic end-to-end ("a Contract consumes a generated frozen extraction"). **Gates ("bring in at time"):** the de-risk spike is anytime on a structured artifact; the full type matures POST-TERMINOLOGY-1 (a richer contract surface to justify generated authoring) and — for the narrative instance — AFTER the locked `SCENARIO-1 → FHIR → StoryWorld` order + the narrative **label model** (the per-bridge labels: by-construction or annotation? annotation → SECOND-CLASS cases, cf. S-BS-104). Memory `jute-generated-contracts-unification`; do NOT author the driver until a gate clears.
...
- **CONTRACT-AUTHOR design** (decide when scoped): should the generated `jute_transform` (extraction) and the generated validators share one registry + lifecycle, and is the trust-model boundary enforced at **freeze-time** (the structural output-invariant gate on the generated extractor) or **per-execution**? Lean: freeze-time — keep the runtime pure-deterministic JUTE (generate-at-authoring, execute-at-runtime).
