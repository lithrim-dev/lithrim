...
> `apps/bff/agent/{tools,loop}.py`, `packs/*/tools.json`, and the published Hermes executor in
> `../lithrim-pack-healthcare/healthcare/floors.py`.
...
- Service executors call a sidecar over HTTP/MCP (e.g. `KbRagTool` → `:8002`; the JUTE connector →
  `:3031`; the pack's Hermes executor → the Hermes MCP). In-process executors run locally (e.g.
  `value_presence`, `record_presence`).
...
`disallowed_tools=["ToolSearch"]`) for BYO-Claude safety (ASAFE-1). New conversational tools are added
...
before it is trusted (the spec lies / the runtime differs) — see `[[jute-extractor-spec-and-live-gate]]`,
`[[harness-jute-is-the-safe-transform-dsl]]`.
...
- **Executor:** `../lithrim-pack-healthcare/healthcare/floors.py` `SnomedSubsumptionGrounding`
...
- **Graceful absence:** Hermes unreachable → fail-clean (the finding stands; no flip). Tests:
  `test_snomed_grounding.py`, `test_specificity_flip.py`.
- **Safety rule:** ground by **code**, not fuzzy search — fuzzy terminology lookup is unsafe
  (`[[kb-grounding-corpus-hipaa-compliancev2]]`).
...
- **Agent-side:** added as an explicit named MCP connector, NOT by flipping `tools=[]` back on
  (preserve the ASAFE-1 tool-less default for BYO-Claude).
...
| Add a conversational (in-process) tool | add a handler in `apps/bff/agent/tools.py` + the SDK-MCP server, per the A-SAFE checklist | BFF-local |
...
- Transport: `service` connectors are external processes — treat their output as untrusted input;
  the model never executes connector-returned code (transforms are JUTE, the safe DSL —
  `[[harness-jute-is-the-safe-transform-dsl]]`).
...
- The JUTE/`:3031` connector (already core) — the importer/transform reference (see the Jute-importer
  cycle).
