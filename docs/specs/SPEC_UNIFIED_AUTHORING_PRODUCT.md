...
**OQ-6 (RESOLVED 2026-06-05 = B; built at UAP-5b).** *Where does the agent loop run?* **(B)** Python Agent SDK in the BFF — in-process, offline, BYO-Claude on desktop, reuses the Python harness/council; cost = a parts-adapter + an SSE endpoint + a new `[agent]` extra → **chosen, on-thesis**; vs **(A)** a TS Vercel-AI-SDK / assistant-ui host in the shell — native parts, no adapter; cost = a second JS runtime + pushes toward API keys, and the local-CLI provider doesn't fit cleanly. **UAP-5b D0 PROVED the BYO-Claude fit:** a trivial `ClaudeSDKClient` + one in-process `@tool` ran on the local `claude` CLI / desktop auth with `ANTHROPIC_API_KEY` unset (no API key). Built as `POST /v1/chat` (SSE) over the CORE tool spine (`author_judge`/`get_judge`/`run_eval`-replay); the full from-scratch journey is the **UAP-5c** fast-follow. See `SPEC_PRODUCT_SHELL.md §10` for the ratified route.

**UAP-5c (BUILT 2026-06-05; grows the spine to the full journey).** The `POST /v1/chat` loop now drives **Domain→Judge→Flag→Run→Review from a blank agent**: three added in-process tools over EXISTING audited/`$0` ops — `get_agent` (Domain read), `author_flag` (an audited **EDIT** of an existing flag, `PUT /v1/ontology`), `review_runs` (run history + provenance) — each adapted to an EXISTING gen-UI card (no new types). The two carried seams closed: **S-BS-82** (the bound `get_judge` FieldInfo break — fixed in the binding wrapper, the op frozen) and **S-BS-81** (A-SAFE proven *safe-by-explicit-bound*: the allowlist is exactly the `mcp__lithrim__*` set with no built-in tool, and no tool schema carries a paid knob). The Review leg renders `tool-audit_log` (pure-read) rather than `tool-run_panel`, whose paid Run is `window.confirm`-gated (S-BS-80). See `SPEC_PRODUCT_SHELL.md §10`.

**UAP-5c-2 (BUILT 2026-06-05; completes the tool set — the pre-authorized D-F split).** The `POST /v1/chat` loop completes to **8 tools** by wrapping the two remaining ops: `run_eval_pack` (the eval-pack **BATCH** — the FIRST tool over a PAID-CAPABLE op; the bound closure HARDCODES `live=False`, and injecting `live=True` into the tool args is **dropped**, negative-tested) and `assemble_agent` (the FIRST agent-reachable Agent **WRITE**, **edit-one-facet** = the judges roster: add/remove ONE known v2 judge per `LENS_BY_ROLE` — an unknown role is refused, never a full dict from the model; the audited `PUT /v1/agent`, `target_type=agent`). Both reuse EXISTING cards (`run_eval_pack→tool-audit_log` pure-read, the batch round-trips to `GET /v1/runs`; `assemble_agent→tool-agent_editor`). A-SAFE re-proven *safe-by-explicit-bound* across the widened surface (the allowlist-bounds + no-paid-knob tests now cover all eight tools); both wrapped ops stay byte-identical (A6 0-delta). See `SPEC_PRODUCT_SHELL.md §10`.

**ASAFE-1 / S-BS-90 (HARDENED 2026-06-06; HARD-GATE) — the A-SAFE floor = the PreToolUse deny gate, not the allowlist value.** The ONB-0 A-LIVE caught the live agent spontaneously running built-in `Bash`: the loop ran `permission_mode="bypassPermissions"`, under which `allowed_tools` governs prompting only, so the `mcp__lithrim__*` allowlist was **never an exclusive bound**. Every "A-SAFE re-proven *safe-by-explicit-bound*" line above (UAP-5c/5c-2) is therefore **necessary-but-INSUFFICIENT** — the allowlist-value tests ran a stub source, never the real SDK loop. The **enforced** floor is a deny-by-default `PreToolUse` hook (`agent/loop.py:_deny_non_lithrim`, the only mechanism that gates every call under bypass; **fail-closed**) bounding the loop to `mcp__lithrim__*`, + SDK isolation (`setting_sources=[]`/`skills=[]`). Proven LIVE (`docs/research/RUN_asafe1_live_2026-06-06.json`: `Bash`/`Read`/`ToolSearch` refused at the tool layer; `get_agent`+`run_eval` still work) + an offline non-vacuous unit (`tests/test_asafe_tool_gate.py`). The 8-tool surface + audited-write path stay FROZEN (A6 0-delta). See `SPEC_PRODUCT_SHELL.md §10`.
