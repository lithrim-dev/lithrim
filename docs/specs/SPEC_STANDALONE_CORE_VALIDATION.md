...
3b. **CE-PACK-6c (follow-on, OPENED by 6b-CLEAN) — the literal grep-clean.** Relocate `build_source_message_prompt` + the source_message evaluate branch + the clinical retrieval helpers into the healthcare pack (the §4-deferred clinical PROMPT residue), and decide `phi_redaction.py`'s fate (keep as a generic-privacy MECHANISM with neutral naming, or relocate to the pack). After 6c, `grep -riE '<needles>' lithrim_bench/runtime/council/` over the non-test core → empty. Gated; FROZEN-adjacent (the source_message branch is in the frozen `compliance_council.py`). Tracked by `tests/test_6bclean_attestation.py::test_enumerated_buckets_are_not_stale` (the list tightens as buckets are cleaned).
