...
LLM-as-judge is the default for evaluating generated artifacts, but on conformance-sensitive tasks in regulated domains it has a blind spot that more judge accuracy does not fix: a semantic judge cannot reliably detect violations of a machine-checkable specification, such as a clinical note with a malformed medication entry or a FHIR Claim with no `provider.reference`, because the judge is reasoning about meaning rather than validating structure. We characterize this categorical blindness and study composing a deterministic structural validator with a semantic LLM-judge council under a worst-of rule, evaluated on a synthetic, deterministically labeled clinical artifact benchmark built from Synthea encounter data with programmatic defect injection. Against a strong tuned-judge baseline that replicates Lail & Markham's criteria-injection + 3-judge ensembling at gpt-4.1, we measure the structural error class the composition recovers, the false-positive cost on clean negatives, and agreement decomposed into a stable decision layer and a stochastic code-attribution layer — reporting distributions over N=10 runs per case rather than single verdicts. As motivation we include one production run where a unanimous three-judge council rated a clinical note faithful and complete while structural validation caught a malformed medication entry and a missing allergies list, worst-of escalating to reject. **On the bench's HL7 ADT^A04 pack — the cleanest demonstration of the categorical claim — a tuned-semantic baseline catches 0/28 structural defects, the deployed structural validator (etlp mapping 26, presence-only) catches 8/28 (+28.6 pp under worst-of), and a stricter conformance validator generated via the etlp-mapper Jute copilot (date-format + value-set + trigger-event-consistency checks) catches all 28 (+100 pp) — the structural axis is recovered. On the four semantic packs at N=10 (size 50, held-out test split, 1380 council invocations), the 3-judge live gpt-4.1 council achieves 0.730 ensemble accuracy, with a 5.73× decision-vs-code-attribution gap (verdict-level 0.722 mean per-judge vs exact-flag 0.126), and `behavior_judge` precision 0.97 / false-block 0.030 (829 reject votes) as the canonical fidelity owner. The same sweep surfaces a pipeline-composition finding the bench made visible: on the scheduling pack, the council unanimously approves all 24 defective test cases (kappa=1.000) while the orchestrator's Stage 2.5 artifact_judge (a single FP-prone gpt-4o-mini voice) carries both the catches and a 0.38 false-block on cleans — reported honestly as orchestrator-composition behavior, not as a structural-axis win.** We release the benchmark generator (lithrim-bench v0.1) and the evaluation harness.
