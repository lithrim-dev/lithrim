...
- **The CI/CD gate** — `lithrim-pack` (reliability ≥ threshold AND
...
> - A CI/CD eval gate (`lithrim-pack`).
>
...
| S-BS-109 | low | The eval-pack **live** endpoint (`/v1/eval-pack/run`, `live=true`) still routes to `:8002` — `build_pack` has no in_process param (a code-acknowledged follow-on). Single-agent "Run live" is standalone; **batched** live is not. | **post-v1** — not shell-exposed (no live-pack button; the chat's pack op is replay-only), and the offline `lithrim-pack` gate is `$0`/standalone. Until the batched in_process follow-on lands, a standalone **batched** live run needs `LITHRIM_COUNCIL_BACKEND=http`. The single-agent standalone claim is unaffected. |
