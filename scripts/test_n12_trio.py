"""N=12 head-to-head: gpt-4o × 3 council (old) vs cross-provider trio (new).

Replicates the 12-case pilot from out/pilot_thesis_n12.ndjson against the new
cross-provider trio:
    - gpt-4.1                                  (Risk Judge,   OpenAI,  logprobs ✓)
    - Mistral-Large-3                          (Policy Judge, Mistral, logprobs ✗)
    - Llama-4-Maverick-17B-128E-Instruct-FP8   (Faithfulness, Meta,    logprobs ✓)

Each model gets the same {transcript + artifact(s)} input as a faithfulness judge,
returns JSON-mode verdict + flags + reasoning. For models that expose logprobs,
we extract the verdict-token logprob to derive calibrated confidence.

Outputs:
    out/pilot_thesis_n12_trio.ndjson         per-row, all 3 models + derived metrics
    out/pilot_thesis_n12_trio.summary.json   aggregate
    out/pilot_thesis_n12_trio.summary.md     head-to-head against old gpt-4o pilot

Cost: ~$0.60-$1.00, wall-clock ~3-5 min.

Usage:
    export AZURE_OPENAI_KEY=...
    PYENV_VERSION=debuglithrim python3 scripts/test_n12_trio.py
"""
...
def main():
    ...
    out_path = REPO_ROOT / "out" / "pilot_thesis_n12_trio.ndjson"
    ...
    (REPO_ROOT / "out" / "pilot_thesis_n12_trio.summary.json").write_text(
        json.dumps(summary, indent=2) + "\n"
    )
    (REPO_ROOT / "out" / "pilot_thesis_n12_trio.summary.md").write_text(render_markdown(rows, summary))

    print(file=sys.stderr)
    print(f"=== Done. {len(rows)} rows. ===", file=sys.stderr)
    print(f"NDJSON:  {out_path}", file=sys.stderr)
    print(f"Summary: {REPO_ROOT / 'out' / 'pilot_thesis_n12_trio.summary.md'}", file=sys.stderr)
    return 0
