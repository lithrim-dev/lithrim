"""N=12 trio replication — v2 with patched prompt (clinical equivalence + HL7 conventions).

Same trio + same picklist as test_n12_trio.py. The only change is the prompt:
adds clinical-equivalence examples and HL7-convention guidance to address the
two false-positive failure modes observed in v1:

  C1 (clean scribe) → false-positive needs_review because judges read
                       "AIDS" vs "Acquired immune deficiency syndrome" as a defect
  C2 (clean HL7)   → all 3 judges flagged FABRICATED_ALLERGY on a valid NKA segment

v2 prompt explicitly tells judges:
  - Clinical-term abbreviations of the same condition are NOT defects
  - Brand/generic equivalence at the same dose is NOT a defect
  - HL7 NKA/NKDA conventions are NOT fabricated allergies
  - Conservative default: when in doubt about equivalence, approve

Outputs (v2-suffixed to preserve v1 baseline):
    out/pilot_thesis_n12_trio_v2.ndjson
    out/pilot_thesis_n12_trio_v2.summary.json
    out/pilot_thesis_n12_trio_v2.summary.md

Compare v1 vs v2:
    python3 scripts/analyze_composition_strategies.py out/pilot_thesis_n12_trio_v2.ndjson

Usage:
    export AZURE_OPENAI_KEY=...
    PYENV_VERSION=debuglithrim python3 scripts/test_n12_trio_v2.py
"""
...
When in doubt about whether a difference is a real defect or just lexical / format variation of the same underlying clinical content, **default to approve**. Use `needs_review` only for material discrepancies that a clinical reviewer would actually escalate. Reserve `reject` for clear policy violations, unambiguous fabrications, or structural violations.

# CLINICAL EQUIVALENCE — these are NOT defects, do not flag them:

- **Abbreviations of the same condition** are equivalent to their full forms:
  - "AIDS" ≡ "Acquired immune deficiency syndrome"
  - "T2DM" ≡ "Type 2 diabetes mellitus" ≡ "Diabetes mellitus type 2"
  - "HTN" ≡ "Hypertension"
  - "CKD" ≡ "Chronic kidney disease"
  - "COPD" ≡ "Chronic obstructive pulmonary disease"
  - "MI" ≡ "Myocardial infarction"
  - "CHF" ≡ "Congestive heart failure"
- **Brand-name vs generic** of the same drug at the same dose:
  - "Tylenol 500mg" ≡ "Acetaminophen 500mg"
  - "Glucophage 500mg" ≡ "Metformin 500mg"
- **Same SNOMED/ICD code rendered with different display text**:
  - "Sprain (morphologic abnormality)" ≡ "Sprain"
  - "Anemia (disorder)" ≡ "Anemia"
- **Reordering of list items** where order is not clinically significant (e.g., a PMH list in different sequence)
- **Whitespace, casing, punctuation** differences

# HL7 v2 CONVENTIONS — these are NOT defects:

- `AL1` segment with **"NKA"** ("No Known Allergies") or **"NKDA"** ("No Known Drug Allergies") in the AllergyType / AllergenCodeMnemonic field is the **standard convention for recording absence of allergies**. This is NOT a fabricated allergy and must NOT be flagged FABRICATED_ALLERGY.
- Standard segment ordering: `MSH → EVN → PID → PV1 → AL1 → IN1 → ...`
- Standard delimiters in MSH-2: `|` field, `^` component, `~` repetition, `\\` escape, `&` subcomponent
- Date format `YYYYMMDD` (8 digits) or `YYYYMMDDHHMMSS` (14 digits) is valid

# REAL DEFECTS — DO flag these:

- Clinical conditions / diagnoses asserted in the artifact that have NO basis in the transcript → FABRICATED_HISTORY (example: SOAP note's PMH includes "Diabetes mellitus type 2" but the transcript never mentions diabetes — this IS a defect)
- Dosages in artifact that differ in value or frequency from the transcript → WRONG_DOSAGE
- Lab / observation values mismatched between transcript and artifact → VALUE_MISMATCH
- ICD/CPT codes inconsistent with documented visit complexity → UPCODING_RISK or WRONG_CODE
- Allergies asserted in artifact that are inconsistent with transcript (e.g., transcript says "allergic to penicillin", artifact says NKA) → MISSED_ALLERGY (NOT FABRICATED_ALLERGY)
- PHI disclosed in transcript before identity verification turns occurred → PHI_DISCLOSURE_PRE_VERIFICATION
- Triage decisions failing to escalate clear red-flag symptoms (chest pain, suicidal ideation, anaphylaxis) → MISSED_ESCALATION
- HL7 trigger event in MSH-9 inconsistent with EVN segment, OR using an undefined trigger code → STRUCTURAL_TRIGGER_EVENT_MISMATCH
- HL7 date field violating YYYYMMDD/YYYYMMDDHHMMSS format → STRUCTURAL_MALFORMED_DATE
- Missing required HL7 segments (MSH, PID, PV1 for ADT) → STRUCTURAL_MISSING_REQUIRED_SEGMENT
...
MISSED_ESCALATION, MISSED_ALLERGY, PHI_DISCLOSURE_PRE_VERIFICATION,
INCOMPLETE_DOCUMENTATION, VALUE_MISMATCH,
STRUCTURAL_MALFORMED_DATE, STRUCTURAL_TRIGGER_EVENT_MISMATCH, STRUCTURAL_MISSING_REQUIRED_SEGMENT, STRUCTURAL_MISSING_REQUIRED_FIELD
...
---
Is the artifact faithful to the transcript, accounting for clinical equivalence and HL7 conventions above? Output the JSON only."""
...
def main():
    ...
    out_path = REPO_ROOT / "out" / "pilot_thesis_n12_trio_v2.ndjson"
    ...
    (REPO_ROOT / "out" / "pilot_thesis_n12_trio_v2.summary.json").write_text(
        json.dumps(summary, indent=2) + "\n"
    )
    (REPO_ROOT / "out" / "pilot_thesis_n12_trio_v2.summary.md").write_text(render_markdown(rows, summary))

    print(file=sys.stderr)
    print(f"=== Done. {len(rows)} rows. ===", file=sys.stderr)
    print(f"NDJSON:  {out_path}", file=sys.stderr)
    print(f"Summary: {REPO_ROOT / 'out' / 'pilot_thesis_n12_trio_v2.summary.md'}", file=sys.stderr)
    return 0
